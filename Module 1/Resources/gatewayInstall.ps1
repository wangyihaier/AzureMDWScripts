...
function Uninstall-OldGateway()
{
    Trace-Log "Check & Uninstall Old Gateway if there is"
	$Gateway = Get-WmiObject -Class Win32_Product | Where-Object{$_.Name -eq "Microsoft Integration Runtime"}
	if($Gateway -eq $null)
	{
		Trace-Log "No Gateway installed on this machine!"
	}
	else
	{
		$Gateway.Uninstall()
		Trace-Log "The old version of Gateway has been uninstalled successfully"
