<#
.SYNOPSIS
  This script initializes an existing Azure Data Lake and Azure SQL DW in preparation for Module 4. Use this script if you haven't previously run Modules 1, 2, and 3 of the Ready Lab

.DESCRIPTION
  This script creates external staging tables in the SQL DW. It also initializes the DW with appropriate logins and users
...
# ----------- Create fireEvents production table if not exists -------------------------------------------------------------------------------------------------
$tblExists = (Invoke-Sqlcmd -Query "SELECT a.* from sys.tables a join sys.schemas b on a.schema_id = b.schema_id where b.name = 'prod' and a.name = 'factFireEvents'" -ServerInstance $fullyQualifiedServerName -Database $dataWarehouseName -Username $adminUser -Password $adminPassword).object_id
if (!$tblExists)
{
    Write-Host "Creating fireEvents production table in data warehouse instance..."
    Invoke-Sqlcmd -InputFile "$PSScriptRoot\CreateFireEventsDistributedTable.sql" -ServerInstance $fullyQualifiedServerName -Database $dataWarehouseName -Username $adminUser -Password $adminPassword
}

Write-Host "All resources initialized for module 4"
